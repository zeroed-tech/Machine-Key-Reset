...
function GetIISMachineKeyLocationForApplicationPool($applicationPool) {
    switch ($applicationPool.ProcessModel.IdentityType) {
        ...
        Default {
            ...
            $keyTime = GetMachineKeyFromKey($keyLocation)
...
function GetMachineKeyFromKey($key) {
...
Get-IISAppPool | ForEach-Object {
    ...
    $appPoolUserName = GetUsernameForApplicationPool($appPool)
    $sid = GetSidForApplicationPool($appPool)
    $iisMachineKeyTime = GetIISMachineKeyLocationForApplicationPool($appPool)

    $res = [pscustomobject]@{
        ApplicationPool = $appPoolName
        UserName        = $appPoolUserName
        SID             = $sid
        MachineKeyFound = $($iisMachineKeyTime -ne $null)
        Created         = $iisMachineKeyTime.Date
        Path            = $iisMachineKeyTime.Path
    }
